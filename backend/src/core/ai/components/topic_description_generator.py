...
from src.core.ai.components.llms import UseLLMsGroqAsync, GroqModelEnum, UseLLMsGroqSync
...
class TopicDescriptionGeneratorAsync:
    def __init__(self, groq_client: UseLLMsGroqAsync = None):
        self.prompts = TopicDescriptionPrompts()
    ...
    async def generate_canonical_name(
        ...
        prompt = self.prompts.CANONICAL_NAME_ONLY_PROMPT.format(user_input=topic)
        try:
    ...
    async def generate_topic_description(
        ...
        prompt = self.prompts.PRIMARY_PROMPT.format(user_input=topic, other_topics=other_topics)
        try:
            result = await self._client.chat_completion(
                prompt=prompt, model=model, temperature=temperature
            )
